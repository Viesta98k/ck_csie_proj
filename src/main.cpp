...
int main(){
    ...
    vector<vector<char>>player_answer(0);
    int player_choice=-1;
    int selected_x=-1, selected_y=-1;   //特殊模式中玩家選擇的格子
    int chosen_alphabet=-1;
    //計時器
        ...
        if(g.in_level){
            if(click_x>1160&&click_x<1280&&click_y>680&&click_y<700){   //退出關卡
                ...
                if(click_x>640-8*x_size&&click_x<640+8*x_size&&click_y>360-8*y_size&&click_y<360+8*y_size){
                    selected_x=(click_x-640+8*x_size)/16;
                    selected_y=(click_y-360+8*y_size)/16;    
                    if(g.current_special_question.giveCorrectAns()[selected_x][selected_y]==NULL){
                        selected_x=-1;
                        selected_y=-1;
                    }
                }else if(click_x>432&&click_x<848&&click_y>620&&click_y<636){ //選字母
                    chosen_alphabet=(click_x-432)/16+41;    //A,B,C...
                    player_answer[selected_x][selected_y]=chosen_alphabet;
                }else{  //取消目前選取
                    selected_x=-1;
                    selected_y=-1;
                }
           ...
           if(end_of_round){
            ...
            if(!is_special_question){
                ...
                for(int i=0;i<g.current_special_question.ret_vocs_num();i++){
                    ...
                    for(int j=0;j<g.current_special_question.ret_voc(i).size();j++){    //對答案
                        if(g.current_special_question.ret_vertical(i)){
